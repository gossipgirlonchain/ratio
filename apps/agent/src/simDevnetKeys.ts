/**
 * Devnet-sim-only helpers: keypair persistence + faucet funding. Same code
 * as @ratio/doppler scripts/helpers.ts (script-side, not exported by the
 * package on purpose — nothing in production loads keys from disk).
 */
...
/** Load a persisted 64-byte keypair file, or generate + persist one. Devnet only. */
...
export async function ensureFunded(
  ...
  if (current < min) {
    throw new Error(
      `Devnet faucet exhausted. Fund ${addr} manually and re-run.`,
    );
  }
  return current;
}
